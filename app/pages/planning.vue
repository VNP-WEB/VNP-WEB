    ...
    <section class="bg-gradient-to-br from-blue-600 to-cyan-600 text-white py-20">
      <div class="container mx-auto px-4">
        <div class="max-w-3xl mx-auto text-center">
          <h1 class="text-4xl md:text-5xl font-bold mb-6">{{ $t('planning.hero_title') }}</h1>
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
          <div class="p-6 border-b flex items-center justify-between bg-white">
            ...
            <div class="flex items-center gap-2">
              ...
              <button @click="resetToToday" class="px-4 py-2 border border-gray-300 rounded hover:bg-gray-50 font-medium text-gray-700 h-10">{{ $t('planning.today') }}</button>
        ...
        <div class="bg-white rounded-3xl shadow-2xl w-full max-w-lg border border-blue-100 overflow-hidden transform transition-all p-0">
          ...
          <div class="p-8 md:p-10 text-center">
            ...
            <div v-if="isSuccess" class="py-6 animate-in zoom-in duration-300">
               ...
               <h3 class="text-3xl font-extrabold text-gray-900 mb-2">{{ $t('planning.success_title') }}</h3>
               <p class="text-gray-600 text-lg">{{ $t('planning.success_desc') }}<br><span class="font-bold text-blue-600">{{ formData.email }}</span></p>
               <div class="mt-8 text-sm text-gray-400">{{ $t('planning.success_footer') }}</div>
            </div>

            <div v-else>
              <h3 class="text-3xl font-bold text-gray-950 mb-6">{{ $t('planning.confirm_title') }}</h3>
              ...
              <div class="bg-blue-50 border border-blue-100 text-blue-900 p-6 rounded-2xl mb-8 text-left">
                <div class="flex items-center gap-4 mb-2">
                   ...
                   <p class="text-xl font-extrabold text-blue-950 capitalize">{{ $t('planning.selected_date', { date: selectedDateDisplay, time: selectedTime }) }}</p>
                </div>
              </div>
              ...
              <form @submit.prevent="submitBooking" class="space-y-6 text-left">
                ...
                <div class="group">
                  <label class="block text-sm font-bold text-gray-700 mb-2">{{ $t('planning.form_name') }}</label>
                  <div class="relative">
                ...
                <div class="group">
                  <label class="block text-sm font-bold text-gray-700 mb-2">{{ $t('planning.form_email') }}</label>
                  <div class="relative">
                    <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-gray-400 group-focus-within:text-blue-500">
                ...
                <div class="group">
                  <label class="block text-sm font-bold text-gray-700 mb-2">{{ $t('planning.form_reason') }}</label>
                  <div class="relative">
                    <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-gray-400 group-focus-within:text-blue-500">
                    ...
                    <input v-model="formData.reason" type="text" required :placeholder="$t('contact.form.subjectPlaceholder')" class="w-full pl-12 p-4 border border-gray-200 bg-gray-50 rounded-2xl outline-none focus:border-blue-300 focus:ring-4 focus:ring-blue-100 transition">
                  </div>
                </div>

                <div class="flex gap-4 mt-10 pt-6 border-t border-gray-100">
                  <button type="button" @click="showModal = false" class="px-6 py-4 border-2 border-gray-100 rounded-2xl text-gray-500 font-bold hover:bg-gray-50 transition">{{ $t('planning.cancel') }}</button>
...
// On identifie la locale exacte pour le formatage des dates JS
  ...
  try {
    ...
    await $fetch('/api/notify-booking', {
      ...
      body: {
        ...
        locale: locale.value // On envoie la langue du client
      }
    })
